...
pesos <- nb2listw(vecinos, style="C", zero.policy=T)
...
ggplot()+
  geom_sf(data=cir)+
  geom_sf(data=filter(cir, CIRCUITO_N==circuito_objetivo), aes(fill="Objetivo"))+
  geom_sf(data=filter(cir, CIRCUITO_N %in% vecinos[circuito_objetivo][[1]]), aes(fill="Vecinos"))+
...
-   `Cuadrante (mean)` : Cuadrante donde podemos clasificar el Moran local.
