---
title: "2. Afinando las herramientas"
author: "Tomás Bustos"
format:
  html:
    toc: true
...
-   Graficar el área para todas las provincias utilizando bordes simplificados. Jugar con el valor de simplificación para que se note la operación.
...
Veamos otra forma de comparar los cortes.

```{r}
asignar_grupos <- function(valor, estilo, n_clases) {
  cortes <- classInt::classIntervals(valor, n = n_clases, style = estilo)$brks
  findInterval(valor, cortes, all.inside = TRUE)
}
...
df_comparativo <- df %>%
  ...
  mutate(
    `Equal` = asignar_grupos(Porcentaje, "equal", k),
    `Quantile` = asignar_grupos(Porcentaje, "quantile", k),
    `Jenks` = asignar_grupos(Porcentaje, "jenks", k)
  ) %>%
  pivot_longer(
    cols = c(contains("Equal"), contains("Quantile"), contains("Jenks")),
    names_to = "metodo",
    values_to = "grupo"
  ) %>%
  mutate(grupo = as.factor(grupo)) %>% 
  mutate(id = stringr::str_replace_all(id, c("CIUDAD AUTONOMA DE BUENOS AIRES"="CABA",
                                             "TIERRA DEL FUEGO"="TDF",
                                             "SANTIAGO DEL ESTERO"="S. DEL ESTERO")))

ggplot(df_comparativo, aes(x = reorder(id, Porcentaje), y = Porcentaje, fill = grupo)) +
  geom_col(width = 0.8) +
  # Agregamos los números cerca de la base (y = 1 o el valor que prefieras)
  geom_text(aes(y = 1, label = round(Porcentaje, 0)), 
            color = "black", 
            angle = 90, 
            hjust = 0, # Alinea el texto hacia arriba desde el punto y=1
            size = 3) +
  facet_grid(metodo ~ .) + 
  scale_fill_brewer(palette = "Purples") +
  labs(
    title = "Comparación de métodos de clasificación",
    y = NULL, # Eliminamos el título del eje Y
    fill = "Grupo"
  ) +
  theme_minimal() +
  theme(
