...
df %>% 
  ...
  ggplot()+
  annotation_map_tile(zoom=9) + # con zoom=8 o más se rompe el renderizado para web. Probar localmente.
  geom_sf(aes(fill=Porcentaje), color="black", alpha=.8)+
...
df %>% 
  ...
  filter(seccion_electoral %in% c("I", "III", "VIII")) %>% 
  ggplot()+
  annotation_map_tile(type="cartodark", zoom=9) + # con zoom=8 o más se rompe el renderizado para web. Probar localmente.
  geom_sf(aes(fill=Porcentaje), color="black", alpha=.8)+
  scale_fill_fermenter(palette = "Blues", direction = 1, n.breaks=3,
                       labels = scales::label_number(suffix = "%"))+ 
  labs(x="", y="", fill="",
...
df_biscale <- df_filtrado %>% 
  ...
  mutate(Electores = as.numeric(Electores)) %>% 
  bi_class(x="Electores", y="Porcentaje", style="quantile", dim=dim)
...
df15 %>% 
  ...
  labs(title="Ganador por departamento",
       subtitle="Elecciones presidenciales B2015")+
  theme_void()
```
...
## Yapa: radios censales y circuitos electorales

```{r}
cir_path = "http://datar.info//dataset/fc99d6a8-9b47-4e9e-9436-58038735a5eb/resource/c836bb95-4e4e-4b33-8578-057175748d89/download/circuitoselectoralescaba.json"
cir <-  st_read(cir_path)
cir

rad_path <- "https://cdn.buenosaires.gob.ar/datosabiertos/datasets/direccion-general-de-estadisticas-y-censos/informacion-censal-por-radio/caba_radios_censales.geojson"
rad <- st_read(rad_path)

ggplot()+
  geom_sf(data=rad, aes(color="Radios censales"), fill=NaN)+
  geom_sf(data=cir, aes(color="Circuitos electorales"), fill=NaN, linewidth=.8)+
  coord_sf(xlim = c(-58.490, -58.430), ylim = c(-34.615, -34.575))+
  scale_color_manual(values = c("Radios censales" = "red", "Circuitos electorales" = "black"),
                     name = "Capas Geográficas") +
  labs(title="Superposición circuitos y radios censales", subtitle="Ciudad de Buenos Aires - Comuna 15")
```
